...
function useFormStatus({ initialDirty, initialTouched, mode, $values }) {
	...
	return {
		...
		resetDirty,
		resetTouched,
		isTouched: (0, react.useCallback)((path) => require_get_status.getStatus(touchedRef.current, path), [touchedRef.current]),
		setFieldTouched,
		setFieldDirty,
		setTouchedState,
		setDirtyState,
		...
		getDirty: (0, react.useCallback)(() => dirtyRef.current, [dirtyRef.current]),
		getTouched: (0, react.useCallback)(() => touchedRef.current, [touchedRef.current]),
